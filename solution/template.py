...
def call_openai(
    ...
    return response.choices[0].message.content, max(time.time() - start, 1e-9)
...
def compare_models(prompt: str) -> dict:
    ...
    return {"gpt4o_response": gpt4o_text, "mini_response": mini_text,
            "gpt4o_latency": gpt4o_latency, "mini_latency": mini_latency,
    ...
    while True:
        ...
        for chunk in stream:
            ...
            try:
                ...
                print(delta.encode("ascii", "backslashreplace").decode(), end="", flush=True)
            reply += delta
        print()
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        rows.append(" | ".join([item["prompt"][:40], item["gpt4o_response"][:40],
            item["mini_response"][:40], f'{item["gpt4o_latency"]:.2f}s',
            f'{item["mini_latency"]:.2f}s']))
    return "\n".join(rows)
